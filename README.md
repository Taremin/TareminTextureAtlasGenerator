...
選択したオブジェクトの Texture Atlas を生成するアドオンです。 (Blender 2.83 以降必須)
...
1. Texture Atlas を生成したいオブジェクトの選択方法を指定します。
  - `Selected Objects` は選択したオブジェクト（複数可能）
  - `All Objects` はすべてのオブジェクト
  - どちらの選択方法でもアーマチュアオブジェクトは無視されます
  - 複数のオブジェクトが対象の場合、そのすべてのオブジェクトのマテリアルからテクスチャアトラスが生成されます
